/*********************************
  ...
  Node Helper for MMM-DarkSkyForecast2.
...
module.exports = NodeHelper.create({
    ...
    socketNotificationReceived: function(notification, payload) {
        if (notification === "DARK_SKY_FORECAST_2_GET") {

            var self = this;

            if (payload.apikey == null || payload.apikey == "") {
                console.log("[MMM-DarkSkyForecast2] " + moment().format("D-MMM-YY HH:mm") + " ** ERROR ** No API key configured. Get an API key at https://darksky.net");
            } else if (payload.latitude == null || payload.latitude == "" || payload.longitude == null || payload.longitude == "") {
                console.log("[MMM-DarkSkyForecast2] " + moment().format("D-MMM-YY HH:mm") + " ** ERROR ** Latitude and/or longitude not provided.");
            } else {
                ...
                // console.log("[MMM-DarkSkyForecast2] Getting data: " + url);
                request({ url: url, method: "GET" }, function(error, response, body) {
                    ...
                    if (!error && response.statusCode == 200) {
                        ...
                        self.sendSocketNotification("DARK_SKY_FORECAST_2_DATA", resp);

                    } else {
                        console.log("[MMM-DarkSkyForecast2] " + moment().format("D-MMM-YY HH:mm") + " ** ERROR ** " + error);
                    }
